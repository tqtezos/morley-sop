...
type family TCode (t :: T) :: [[T]] where
  ...
  -- TCode ('TOption t) = '[] ': TCode t
  TCode ('TOption t) = '[ '[ 'TOption t]]
...
--
-- See `fromValue'`
toValue' :: forall instr t. SingI t => SOP (Value' instr) (TCode t) -> Value' instr t
toValue' xs =
  case sing @t of
    STc _ct ->
      case xs of
...
-- a `SOP` form.
fromValue' :: forall instr t. SingI t => Value' instr t -> SOP (Value' instr) (TCode t)
fromValue' (VC xs) =
  case sing @t of
...
fromValue' (VContract xs) =
  case sing @t of
    (STContract _p) -> SOP $ SOP.Z $ (SOP.:* SOP.Nil) $ (VContract xs)
fromValue' (VPair (xs, ys)) =
  case sing @t of
    STPair _sl _sr ->
